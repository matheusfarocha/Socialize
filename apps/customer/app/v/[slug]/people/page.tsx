...
function buildMockUsers(venueId: string, tables: TableSummary[]): ActiveUserRecord[] {
  const now = new Date().toISOString();
  const t = (i: number) => tables[i]?.id ?? null;
  return [
    { id: "mock-1", venueId, userId: "u1", sessionId: "s1", initials: "JR", occupation: "UX Designer", interests: "Coffee tasting, sketching, design systems", tableId: t(0), lastSeen: now, status: "active" },
    { id: "mock-2", venueId, userId: "u2", sessionId: "s2", initials: "AL", occupation: "Grad Student", interests: "Machine learning, board games, lo-fi music", tableId: t(0), lastSeen: now, status: "active" },
    { id: "mock-3", venueId, userId: "u3", sessionId: "s3", initials: "KW", occupation: "Freelance Writer", interests: "Creative fiction, people watching, flat whites", tableId: t(1), lastSeen: now, status: "idle" },
    { id: "mock-4", venueId, userId: "u4", sessionId: "s4", initials: "DP", occupation: "Product Manager", interests: "Startups, hiking, specialty lattes", tableId: t(2), lastSeen: now, status: "active" },
    { id: "mock-5", venueId, userId: "u5", sessionId: "s5", initials: "SN", occupation: "Photographer", interests: "Street photography, matcha, vinyl records", tableId: t(1), lastSeen: now, status: "active" },
    { id: "mock-6", venueId, userId: "u6", sessionId: "s6", initials: "MT", occupation: "Software Engineer", interests: "Open source, espresso, rock climbing", tableId: t(3), lastSeen: now, status: "active" },
    { id: "mock-7", venueId, userId: "u7", sessionId: "s7", initials: "RB", occupation: "Music Producer", interests: "Synths, cold brew, late night jams", tableId: null, lastSeen: now, status: "idle" },
    { id: "mock-8", venueId, userId: "u8", sessionId: "s8", initials: "EV", occupation: "Yoga Instructor", interests: "Mindfulness, herbal tea, travel stories", tableId: t(2), lastSeen: now, status: "active" },
  ];
}
...
function PeoplePageContent() {
  ...
  const refreshUsers = useCallback(
    async (force = false) => {
      ...
      if (error) {
        const backendMissing = isPresenceBackendMissing(error.message);
        setPresenceRosterReady(!backendMissing);
        if (backendMissing) {
          const mock = buildMockUsers(venue.id, tables);
          userCacheRef.current = { timestamp: now, data: mock };
          setActiveUsers(mock);
          setPageError("");
        } else {
      ...
      if (nextUsers.length === 0) {
        nextUsers = buildMockUsers(venue.id, tables);
      }
